...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into XIJJY_yufa (title,link) values (%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__=="__main__":
    for num in range(1,162):
        print(num)
        url = "https://jp.xsjedu.org/Topic/yufa/" + str(num) + "/"
        html = call_page(url)
