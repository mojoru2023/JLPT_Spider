...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into YH (title,links) values (%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    ...
    # url = 'http://language.koolearn.com/jp/yufa/'

    for num in range(1,112):
        url = 'http://language.koolearn.com/jp/yufa/' + str(num)+ '.html'
